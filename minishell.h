...
typedef enum e_qstate { Q_NONE = 0, Q_SINGLE, Q_DOUBLE } t_qstate;
...
typedef struct s_token {
  ...
  bool no_expand; /* true → heredoc delimiter, skip $VAR expansion */
  struct s_token *next;
} t_token;
...
typedef enum e_redirtype { RD_IN = 1, RD_OUT_TRUNC, RD_OUT_APPEND } t_redirtype;
...
char **ft_split(char const *s,
                char c); // split.c ile uyumlu olmalı (tek char delimiter)
