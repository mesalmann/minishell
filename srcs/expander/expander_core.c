...
static bool	expand_dollar(t_ctx *ctx, t_expbuf *eb, const char *in)
{
	...
	if (in[eb->i] == '?')
		...
		val = ft_itoa(getpid());
	else if (ms_is_var_start(in[eb->i]))
		return (expand_var_name(ctx, eb, in));
	else if (in[eb->i] >= '0' && in[eb->i] <= '9')
	{
		eb->i++;
		return (true);
	}
	...
	if (!val || !ms_sb_pushs(&eb->out, &eb->len, &eb->cap, val))
	{
		free(val);
		return (false);
	}
...
static bool	process_char(t_ctx *ctx, t_expbuf *eb, const char *in, bool in_dq)
{
	if (!in_dq && handle_quote_toggle(eb, in[eb->i]))
		return (true);
...
char	*ms_expand_str(t_ctx *ctx, const char *in, bool in_dquote)
{
	...
	while (in && in[eb.i])
	{
		if (!process_char(ctx, &eb, in, in_dquote))
		{
			free(eb.out);
			return (NULL);
		}
